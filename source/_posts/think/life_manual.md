...
- 暂无终止期限，发现问题随时调整。
- 背景
    - 本文总结经验，就是为了用于指导自己做事。随着它包含的东西越来多，即使不时复习本文、自省，但是在实际生活中，我也很难面面俱到地遵守、执行好它们。这给我造成了很大的心理压力，焦虑…… 毕竟都是些很复杂、困难的事。
    - 我希望能够有显著、坚实的进步，所以迫切需要具有可行性的计划，以及实际的行动。
    - 于是我尝试一个个完全内化这些经验、要求，让它们变成习惯：以后潜意识就能作出正确的选择，然后自然、正确地做事。
- 计划：一次养成一个习惯，至少坚持 100 天
    - 暂定实际执行时，同时养成两个不互相冲突（需要同时关注两个不同方面的问题）的习惯。
        - __番茄时间不分神__（专注）
        - __早睡早起__
    - 在 Mac 便签 App 里记录、分析、改进执行情况。
