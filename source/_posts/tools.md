...
- [__TotalFinder__](https://totalfinder.binaryage.com/)
    macOS 自带的 Finder（文件管理器）的功能增强插件。
...
- [__搜狗输入法__](https://pinyin.sogou.com/mac/)
    最重要的是：云输入（词库）、双拼（小鹤）。
    - 智能英语输入模式（单词候选，可以提示拼写）
    - U模式：
    - V模式：
    - U模式、V模式这些功能其实挺常见的，但各个输入法通常都只在 Windows 平台有实现，macOS 就不行了。
    - 弹窗比较烦人，不过毕竟通过设置还能关掉。
    - 在中文世界，搜狗的输入法虽然也是不够让人满意，但是已经是最好的选择。
    - 以前用 [__百度输入法__](https://shurufa.baidu.com/)
